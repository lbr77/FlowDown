...
extension ShortcutTool: Syncable, SyncQueryable {
    package static let SyncQuery: SyncQueryProperties = .init(
        objectId: ShortcutTool.Properties.objectId.asProperty(),
        creation: ShortcutTool.Properties.creation.asProperty(),
        modified: ShortcutTool.Properties.modified.asProperty(),
        removed: ShortcutTool.Properties.removed.asProperty()
    )
    ...
    private func queryUploadQueueRealObject(_ objects: [UploadQueue], handle: Handle? = nil) {
        ...
        for object in objects {
            ...
            case ShortcutTool.tableName:
                object.realObject = getObject(ShortcutTool.self, objectId: object.objectId, handle: handle)
            default: continue
            }
        }
    }
